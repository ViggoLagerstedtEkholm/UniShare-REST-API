...
if(Session::exists(SESSION_USERID)){
  echo str_replace('---ID---', $forum["forumID"] , $fragments[1]);
}
...
foreach($posts as $post){
  ...
  $temp = str_replace('---userImage---', 'data:image/jpeg;base64,'.base64_encode($post["userImage"]), $fragments[2]);
...
$temp = str_replace('---page---', $page, $fragments[6]);
echo str_replace('---ID---', $forum["forumID"], $temp);
